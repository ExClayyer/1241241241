const productsData = (window.veloData && window.veloData.mockProducts) || [];
const statusLabelMap = (window.veloData && window.veloData.statusLabels) || {
    in_stock: 'В наличии',
    on_order: 'Под заказ'
};
...
function renderSidebar() {
    var counts = { status: {}, type: {}, brand: {}, frameSize: {} };
    var minPrice = Infinity;
    var maxPrice = 0;

    state.products.forEach(function (p) {
        if (p.status)     counts.status[p.status] = (counts.status[p.status] || 0) + 1;
        if (p.type)       counts.type[p.type] = (counts.type[p.type] || 0) + 1;
        if (p.brand)      counts.brand[p.brand] = (counts.brand[p.brand] || 0) + 1;
        if (p.frameSize)  counts.frameSize[p.frameSize] = (counts.frameSize[p.frameSize] || 0) + 1;
        if (p.price != null) {
    ...
    var html = filterGroup('Наличие', 'status', counts.status, 'availability');
    html += filterGroup('Тип', 'type', counts.type, 'types');
    html += '<div class="filter-section ' + (state.uiOpen.price ? 'active' : '') + '">' +
        ...
        '</div>' +
    '</div>';
    html += filterGroup('Бренд', 'brand', counts.brand, 'brands');
    html += filterGroup('Размер рамы', 'frameSize', counts.frameSize, 'frameSize');
...
function bindSidebarEvents() {
    ...
    if (pMin && pMax) {
        function onPriceChange() {
            const minVal = Number(pMin.value) || 0;
            const maxVal = Number(pMax.value) || 0;
...
function render() {
    ...
    el.grid.innerHTML = result.map(function (item, i) {
        const inCart = window.veloData ? window.veloData.isInCart(item.name) : false;
        const inStock = item.status === 'in_stock';
        const imgNum = (productsData.indexOf(item) % 7) + 1;
